...
.rev-topic-name { font-family: var(--font-head); font-size: 22px; font-weight: 800; margin-bottom: 8px; }
...
<nav id="topnav">
  ...
  <div class="nav-right">
    <button class="nav-btn" onclick="openPomodoro()" title="Pomodoro Timer">🍅</button>
    <button class="nav-btn" onclick="openScheduleImport()" title="Load Daily Schedule">📅 Schedule</button>
    <button class="nav-btn" onclick="openJsonImport()" title="Import Syllabus">⬆ Import</button>
    <div class="sync-badge" id="sync-status">☁️ Connecting</div>
    <button class="nav-btn" onclick="openSettingsPanel()" title="Settings & Backup">⚙️ Settings</button>
    <button id="theme-toggle-btn" onclick="toggleTheme()" title="Toggle Light/Dark Theme">☀️</button>
    <div id="timer-display" onclick="toggleTimer()" title="Click to start/stop">⏱ 00:00:00</div>
    <!-- Auth UI -->
...
<div id="view-folders" class="view">
  <div class="view-header">
    ...
    <div style="display:flex;gap:8px;margin-left:auto;flex-wrap:wrap">
      <button class="btn btn-secondary btn-sm" onclick="openAddFolder()">+ Add Folder</button>
...
<div id="view-subfolder" class="view">
  <div class="view-header">
    ...
    <div>
      ...
      <div class="view-subtitle" id="subfolder-view-subtitle"></div>
    </div>
    <div style="margin-left:auto;display:flex;gap:8px">
      <button class="btn btn-secondary btn-sm" id="add-subfolder-btn" onclick="openAddSubfolder()">+ Subfolder</button>
...
<div class="modal-overlay" id="schedule-import-overlay" onclick="if(event.target===this)closeScheduleImport()">
  <div class="modal modal-lg">
    ...
    <div class="modal-subtitle">Load an AI-generated JSON schedule for today — it renders as a visual timeline and auto-adds topics to your subject folders.</div>
...
<div class="modal-overlay" id="add-subfolder-overlay" onclick="if(event.target===this)closeAddSubfolder()">
  <div class="modal">
    ...
    <div class="modal-title">📂 Add Subfolder</div>
    ...
    <div class="modal-footer">
      ...
      <button class="btn btn-primary" onclick="saveAddSubfolder()">Create Subfolder</button>
    </div>
  </div>
</div>
...
'use strict';
/* =============================================================
   MEDTRACK PRO v2 — Complete Medical Student Command Center
   Automated Spaced Repetition | Canvas Firebase Backend | Analytics
============================================================= */
...
async function initFirebase() {
  try {
    ...
    // If Canvas injected a custom token use it; otherwise restore previous session or stay unauthed
    if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
      await signInWithCustomToken(auth, __initial_auth_token);
    }
    // onAuthStateChanged fires immediately with persisted session (Google or anonymous)
    onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        // Update Firestore path to use the persistent UID (works for Google AND anonymous)
        ...
        unsubscribeSnapshot = onSnapshot(stateRef, (docSnap) => {
          if (docSnap.exists()) {
            ...
            // Deep merge: remote wins, but keep local dailySchedule if remote has none
...
async function saveToFirestore() {
  ...
  try {
    // Serialize entire state object into a stable string to guarantee deeply nested folders/topics 
    // never hit generic Firebase nested array rejection boundaries 
...
function updateSyncStatus(status) {
  ...
  if (!el) return;
  const map = {
    synced:  { text:'☁️ Cloud Synced', cls:'synced'  },
    syncing: { text:'🔄 Saving…',     cls:'syncing' },
    error:   { text:'⚠️ Sync Error',  cls:'error'   },
    offline: { text:'📴 Offline',     cls:'error'   },
    loading: { text:'⬇️ Loading…',    cls:'syncing' }
  };
...
// Fixed Intervals for Automated Science-Backed Spaced Repetition
...
function createTopicData(title, folderId, opts = {}) {
  return {
    id: genId(),
    name: title,
    folderId,
    subfolderID: opts.subfolderID || null,
...
function getAllTopics() {
  ...
  state.folders.forEach(f => {
    f.topics.forEach(t => all.push({ topic:t, folder:f, subfolder:null }));
    (f.subfolders||[]).forEach(sf => sf.topics.forEach(t => all.push({ topic:t, folder:f, subfolder:sf })));
  });
  return all;
}
...
function getFolder(fid) { return state.folders.find(f => f.id === fid); }
function getSubfolder(fid, sfid) {
  const f = getFolder(fid);
  return f ? (f.subfolders||[]).find(sf => sf.id === sfid) : null;
}

// ─────────────────────────────────────────────
//  AUTOMATED SPACED REPETITION (SCIENCE-BACKED)
// ─────────────────────────────────────────────
...
function getWeeklyStudiedTopics() {
  ...
  const results = [];
  state.folders.forEach(f => {
    (f.topics || []).forEach(t => { if (ids.has(t.id)) results.push({ topic: t, folder: f, subfolder: null }); });
    (f.subfolders || []).forEach(sf => { (sf.topics || []).forEach(t => { if (ids.has(t.id)) results.push({ topic: t, folder: f, subfolder: sf }); }); });
  });
  return results;
}
...
function navigateToFolder(folderId, subfolderID = null) {
...
function renderHomeStats() {
  ...
  grid.innerHTML = `
    ...
    <div class="stat-card c-blue"><div class="stat-label">Folders</div><div class="stat-value">${state.folders.length}</div><div class="stat-sub">subjects</div><div class="stat-icon">📁</div></div>
...
function renderDueToday() {
  ...
  list.innerHTML = due.slice(0,12).map(({topic, folder}) => {
    ...
    return `<div class="due-topic-pill ${isOverdue?'overdue':''}" onclick="navigateToFolder('${folder.id}')">
...
function renderFolders() {
  ...
  grid.innerHTML = state.folders.map(f => {
    const allTopics = [
      ...(f.topics||[]),
      ...((f.subfolders||[]).flatMap(sf => sf.topics||[]))
    ];
    ...
    const sfCount = (f.subfolders||[]).length;
    ...
    <div class="folder-card" onclick="navigateToFolder('${f.id}')" style="--card-color:${f.color}">
      ...
      <div class="folder-stats">
        ...
        ${sfCount > 0 ? `<span>📂 ${sfCount} sections</span>` : ''}
      </div>
  ...
  if (state.folders.length === 0) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1">
      ...
      <button class="btn btn-primary" onclick="openAddFolder()">+ Create Folder</button>
    </div>`;
  }
}
...
function renderSubfolderView() {
  ...
  const subEl = document.getElementById('subfolder-view-subtitle');
  const bcEl = document.getElementById('subfolder-breadcrumb');

  if (currentSubfolderId) {
    const sf = getSubfolder(currentFolderId, currentSubfolderId);
    if (!sf) { currentSubfolderId = null; renderSubfolderView(); return; }
    if (titleEl) titleEl.textContent = sf.name;
    if (subEl) subEl.textContent = `Inside ${folder.name}`;
    if (bcEl) bcEl.innerHTML = `
      <span class="breadcrumb-item" onclick="showView('folders')">📁 Folders</span>
      <span class="breadcrumb-sep">›</span>
      <span class="breadcrumb-item" onclick="navigateToFolder('${folder.id}')">${folder.icon} ${folder.name}</span>
      <span class="breadcrumb-sep">›</span>
      <span class="breadcrumb-item active">${sf.name}</span>`;
    document.getElementById('add-subfolder-btn').style.display = 'none';
    renderTopicList(sf.topics);
    document.getElementById('subfolder-list-container').innerHTML = '';
  } else {
    if (titleEl) titleEl.textContent = `${folder.icon} ${folder.name}`;
    if (subEl) subEl.textContent = folder.label ? `${folder.label} · ${(folder.topics||[]).length + (folder.subfolders||[]).flatMap(sf=>sf.topics).length} topics` : '';
    if (bcEl) bcEl.innerHTML = `
      <span class="breadcrumb-item" onclick="showView('folders')">📁 Folders</span>
      <span class="breadcrumb-sep">›</span>
      <span class="breadcrumb-item active">${folder.icon} ${folder.name}</span>`;
  ...
  container.innerHTML = `
    ...
    <div class="subfolder-list">
      ${folder.subfolders.map(sf => {
        const total = sf.topics.length;
        const mastered = sf.topics.filter(t => t.stages.mastered).length;
        const due = sf.topics.filter(t => t.nextReviewDate && t.nextReviewDate <= today).length;
        return `<div class="subfolder-item" onclick="navigateToFolder('${folder.id}','${sf.id}')">
    ...
    <div class="section-title" style="margin-top:16px">📑 Direct Topics</div>
  `;
}
...
function findTopic(topicId) {
  for (const f of state.folders) {
    const t = f.topics.find(t => t.id === topicId);
    if (t) return { topic: t, folder: f, subfolder: null };
    for (const sf of (f.subfolders||[])) {
      const t2 = sf.topics.find(t => t.id === topicId);
      if (t2) return { topic: t2, folder: f, subfolder: sf };
    }
  }
  return null;
}
...
function deleteTopic(topicId) {
  ...
  state.folders.forEach(f => {
    ...
    (f.subfolders||[]).forEach(sf => { sf.topics = sf.topics.filter(t => t.id !== topicId); });
  });
  saveState();
  const folder = getFolder(currentFolderId);
  const topics = currentSubfolderId ? getSubfolder(currentFolderId, currentSubfolderId)?.topics || [] : folder?.topics || [];
...
function openEditFolder(fid) {
  ...
  document.getElementById('add-folder-color').value = f.color;
...
function deleteFolder(fid) {
  const f = getFolder(fid);
  if (!f) return;
  if (!confirm(`Delete folder "${f.name}" and all its topics? This cannot be undone.`)) return;
...
function saveAddSubfolder() {
  ...
  f.subfolders.push({ id: genId(), name, topics: [] });
...
function renderRevisionView() {
  ...
  const queueHtml = due.slice(0, 10).map((item, i) => `
    <div class="rev-queue-item ${i===0?'current':''}">
...
function buildRevCard({ topic, folder, subfolder }) {
  return `
    <div class="revision-session-card">
      <div class="rev-topic-name">${topic.name}</div>
      <div class="rev-topic-folder">${folder.icon} ${folder.name}${subfolder?` › ${subfolder.name}`:''}</div>
...
function renderWeekendRevision() {
  ...
  if (!isWeekend()) {
    ...
    grid.innerHTML = items.slice(0, 6).map(({ topic, folder }) => `
      <div class="weekend-topic-chip" onclick="navigateToFolder('${folder.id}')">
  ...
  grid.innerHTML = items.map(({ topic, folder }) => {
    ...
    const pct = getTopicCompletion(topic);
    return `<div class="weekend-topic-chip ${done ? 'reviewed' : ''}" onclick="navigateToFolder('${folder.id}')">
...
function renderWkndSessionCard() {
  ...
  const { topic, folder, subfolder } = _wkndQueue[_wkndIdx];
  const total = _wkndQueue.length;
  ...
  body.innerHTML = `
    ...
    <div class="wknd-session-card">
      ...
      <div class="wknd-session-folder">${folder.icon} ${folder.name}${subfolder ? ' › ' + subfolder.name : ''}</div>
...
function renderWeakStrong() {
  ...
  if (wl) wl.innerHTML = weak.length === 0 ? '<div style="color:var(--text-dim);font-size:12px">No weak topics flagged</div>' : weak.slice(0,8).map(({topic,folder}) => `<div class="ws-item"><span style="color:var(--red)">⚠️</span><span style="flex:1">${topic.name}</span><span style="color:var(--text-muted);font-size:11px">${folder.name}</span></div>`).join('');
  if (sl) sl.innerHTML = strong.length === 0 ? '<div style="color:var(--text-dim);font-size:12px">No mastered topics yet</div>' : strong.slice(0,8).map(({topic,folder}) => `<div class="ws-item"><span style="color:var(--green)">✅</span><span style="flex:1">${topic.name}</span><span style="color:var(--text-muted);font-size:11px">${folder.name}</span></div>`).join('');
}
...
function renderSubjectChart() {
  ...
  const data = state.folders.map(f => {
    const all = [...f.topics,...(f.subfolders||[]).flatMap(sf=>sf.topics)];
...
function importJson() {
  ...
  pendingJsonData.folders.forEach(rawFolder => {
    ...
    if (!existingFolder) {
      existingFolder = { id: genId(), name: rawFolder.name, icon: rawFolder.icon || '📁', color: rawFolder.color || '#00D2BE', label: rawFolder.label || '', subfolders: [], topics: [] };
...
function doSearch() {
  ...
  state.folders.forEach(f => {
    ...
    f.topics.forEach(t => { if (fuzzyMatch(t.name, q)) results.push({ type:'topic', icon:'📑', name:t.name, sub:f.name, action:`MapsToFolder('${f.id}');setTimeout(()=>scrollToTopic('${t.id}'),300)` }); });
    (f.subfolders||[]).forEach(sf => {
      if (fuzzyMatch(sf.name, q)) results.push({ type:'subfolder', icon:'📂', name:sf.name, sub:f.name, action:`MapsToFolder('${f.id}','${sf.id}')` });
      sf.topics.forEach(t => { if (fuzzyMatch(t.name, q)) results.push({ type:'topic', icon:'📑', name:t.name, sub:`${f.name} › ${sf.name}`, action:`MapsToFolder('${f.id}','${sf.id}');setTimeout(()=>scrollToTopic('${t.id}'),300)` }); });
    });
  });
...
function scrollToTopic(id) { const el = document.getElementById('tc-' + id); if (el) el.scrollIntoView({ behavior:'smooth', block:'center' }); }
...
async function signInWithGoogle() {
  ...
  try {
    ...
    // Before signing in, snapshot local state so we can merge after
    const localSnap = JSON.stringify(state);
    const result = await signInWithPopup(auth, provider);
    // onAuthStateChanged will fire and handle Firestore subscription
    showToast(`✅ Signed in as ${result.user.displayName || result.user.email}!`, 'success');
    closeAuthModal();
    // Save the current local state up to Firestore under the new UID immediately
    // This ensures local work (done offline/anonymous) is NOT lost on first Google login
    ...
    if (e.code === 'auth/unauthorized-domain') {
      ...
      if (window.location.protocol === 'file:' || !domain) {
        if (sub) {
          ...
          You must run this file through a local web server (like VS Code Live Server) so it opens via <strong>http://localhost</strong> or <strong>http://127.0.0.1</strong> (which are authorized by default).`;
        }
      } else {
        if (sub) {
          ...
          Click "Add domain" and copy-paste exactly this:<br>
          <strong style="color:var(--bg);user-select:all;background:var(--primary);padding:4px 8px;border-radius:4px;display:inline-block;margin-top:8px">${domain}</strong>`;
        }
      }
...
function processScheduleFile(file) {
  ...
  reader.onload = (ev) => {
    try {
      ...
      // Show schema hint with parsed info
      if (schema) {
...
function importScheduleToTrack() {
  ...
  // Build schedule items with stable IDs
  const items = pendingScheduleData.schedule.map((s, idx) => ({
    id: s.id || (genId()),
    time: s.time || '',
    subject: s.subject || '',
  ...
  // Auto-add topics into matching subject folders
  let topicsAdded = 0;
  items.forEach(slot => {
    if (!slot.subject || !slot.topic) return;
    // Find or create folder
    let folder = state.folders.find(f => f.name.toLowerCase() === slot.subject.toLowerCase());
    if (!folder) {
      folder = {
        id: genId(), name: slot.subject,
        icon: slot.emoji || '📁',
        color: '#3A86FF',
        label: '', subfolders: [], topics: []
      };
      state.folders.push(folder);
    }
    // Add topic if not already present
    const exists = folder.topics.some(t => t.name.toLowerCase() === slot.topic.toLowerCase()) ||
      (folder.subfolders || []).some(sf => sf.topics.some(t => t.name.toLowerCase() === slot.topic.toLowerCase()));
    if (!exists) {
      const newTopic = createTopicData(slot.topic, folder.id, {
        difficulty: slot.difficulty,
        weightage: slot.weightage,
        tags: slot.tags
      });
      newTopic.notes = slot.notes || '';
      folder.topics.push(newTopic);
      topicsAdded++;
    }
  });

  // Add schedule items as today's planner tasks
  items.forEach(slot => {
    const taskText = `${slot.emoji} [${slot.time}] ${slot.topic} (${slot.subject})`;
